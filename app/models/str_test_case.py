...
class StrTestCase(SQLModel, table=True):
    ...
    plan_key: str = Field(max_length=100, index=True, default_factory=lambda: str(uuid.uuid4()),description="测试计划的key")
    case_key: str = Field(max_length=100, index=True, default_factory=lambda: str(uuid.uuid4()),description="测试步骤的的key")
